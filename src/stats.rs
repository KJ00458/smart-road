...
impl Statistics {
    ...
    pub fn record_spd(&mut self, mx: f64, mn: f64) {
        ...
        self.vel_sum += mx; self.vel_n += 1;
    }
    pub fn record_time(&mut self, t: f64) {
        if t > self.max_time { self.max_time = t; }
        if t < self.min_time { self.min_time = t; }
    }
    pub fn avg_spd(&self)      -> f64 { if self.vel_n==0 {0.0} else {self.vel_sum/self.vel_n as f64} }
    pub fn min_spd_d(&self)    -> f64 { if self.min_spd==f64::MAX {0.0} else {self.min_spd} }
    pub fn min_time_d(&self)   -> f64 { if self.min_time==f64::MAX {0.0} else {self.min_time} }
    pub fn session_secs(&self) -> f64 { self.session.elapsed().as_secs_f64() }
}
