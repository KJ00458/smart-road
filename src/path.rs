...
//! │    col 2 x=520  left-turn lane   → exits West  (row 0, y=400)       │
...
//! │    Facing east, right side = LOW y rows                              │
//! │    row 0 y=400  right-turn lane  → exits North (col 0, x=400)       │
//! │    row 1 y=460  straight lane    → exits East  (x=IX+ROAD+OFF)      │
//! │    row 2 y=520  left-turn lane   → exits South (col 2, x=520)       │
//! │    Spawn x = IX - OFF = 290  (off-screen left)                       │
//! │                                                                       │
//! │  EAST arm — cars enter RIGHT, travel WEST (←):                      │
//! │    Facing west, right side = HIGH y rows                             │
//! │    row 5 y=700  right-turn lane  → exits South (col 5, x=700)       │
//! │    row 4 y=640  straight lane    → exits West  (x=IX-OFF)            │
//! │    row 3 y=580  left-turn lane   → exits North (col 3, x=580)       │
//! │    Spawn x = IX + ROAD + OFF = 810 (off-screen right)               │
//! └──────────────────────────────────────────────────────────────────────┘
//!
//! Outbound (exit) lanes are the opposite side of each arm:
//!   North outbound (going ↑): cols 3,4,5  x=580,640,700
//!   South outbound (going ↓): cols 0,1,2  x=400,460,520
//!   West  outbound (going ←): rows 3,4,5  y=580,640,700
//!   East  outbound (going →): rows 0,1,2  y=400,460,520
...
// West inbound (travelling east → right side = low y)
const W_RT: f64 = 400.0; // row 0 — right turn
const W_ST: f64 = 460.0; // row 1 — straight
const W_LT: f64 = 520.0; // row 2 — left turn

// East inbound (travelling west → right side = high y)
const E_RT: f64 = 700.0; // row 5 — right turn
const E_ST: f64 = 640.0; // row 4 — straight
const E_LT: f64 = 580.0; // row 3 — left turn

// ── Outbound lane centres (opposite half of each arm) ──
// North outbound (cars exiting upward): cols 3,4,5 → x=580,640,700
// South outbound (cars exiting downward): cols 0,1,2 → x=400,460,520
// West outbound (cars exiting leftward): rows 3,4,5 → y=580,640,700
// East outbound (cars exiting rightward): rows 0,1,2 → y=400,460,520

// ── NORTH arm (spawn top y=290, travel South ↓) ──────────────────────────
// Right turn → col 0 (x=400), pivot into East outbound row 0 (y=400)
const PATH_N_R: &[(f64,f64)] = &[
    (N_RT, SPAWN_N),   // (400, 290) spawn
    (N_RT, W_RT),      // (400, 400) pivot row — East outbound row 0
    (EXIT_E, W_RT),    // (810, 400) exit east
];
...
// Left turn → col 2 (x=520), cross, pivot into West outbound row 3 (y=580)
const PATH_N_L: &[(f64,f64)] = &[
    (N_LT, SPAWN_N),   // (520, 290)
    (N_LT, E_LT),      // (520, 580) pivot row — West outbound row 3
    (EXIT_W, E_LT),    // (290, 580) exit west
];

// ── SOUTH arm (spawn bottom y=810, travel North ↑) ───────────────────────
// Right turn → col 5 (x=700), pivot into West outbound row 5 (y=700)
const PATH_S_R: &[(f64,f64)] = &[
    (S_RT, SPAWN_S),   // (700, 810) spawn
    (S_RT, E_RT),      // (700, 700) pivot row — West outbound row 5
    (EXIT_W, E_RT),    // (290, 700) exit west
];
...
// Left turn → col 3 (x=580), cross, pivot into East outbound row 2 (y=520)
const PATH_S_L: &[(f64,f64)] = &[
    (S_LT, SPAWN_S),   // (580, 810)
    (S_LT, W_LT),      // (580, 520) pivot row — East outbound row 2
    (EXIT_E, W_LT),    // (810, 520) exit east
];

// ── WEST arm (spawn left x=290, travel East →) ───────────────────────────
// Right turn → row 0 (y=400), pivot into North outbound col 0 (x=400)
const PATH_W_R: &[(f64,f64)] = &[
    (SPAWN_W, W_RT),   // (290, 400) spawn
    (N_RT, W_RT),      // (400, 400) pivot col — North outbound col 0
    (N_RT, EXIT_N),    // (400, 290) exit north
];
// Straight → row 1 (y=460), continue east
const PATH_W_F: &[(f64,f64)] = &[
    (SPAWN_W, W_ST),   // (290, 460)
    (EXIT_E, W_ST),    // (810, 460)
];
// Left turn → row 2 (y=520), cross, pivot into South outbound col 2 (x=520)
const PATH_W_L: &[(f64,f64)] = &[
    (SPAWN_W, W_LT),   // (290, 520)
    (N_LT, W_LT),      // (520, 520) pivot col — South outbound col 2
    (N_LT, EXIT_S),    // (520, 810) exit south
];

// ── EAST arm (spawn right x=810, travel West ←) ──────────────────────────
// Right turn → row 5 (y=700), pivot into South outbound col 5 (x=700)
const PATH_E_R: &[(f64,f64)] = &[
    (SPAWN_E, E_RT),   // (810, 700) spawn
    (S_RT, E_RT),      // (700, 700) pivot col — South outbound col 5
    (S_RT, EXIT_S),    // (700, 810) exit south
];
// Straight → row 4 (y=640), continue west
const PATH_E_F: &[(f64,f64)] = &[
    (SPAWN_E, E_ST),   // (810, 640)
    (EXIT_W, E_ST),    // (290, 640)
];
// Left turn → row 3 (y=580), cross, pivot into North outbound col 3 (x=580)
const PATH_E_L: &[(f64,f64)] = &[
    (SPAWN_E, E_LT),   // (810, 580)
    (S_LT, E_LT),      // (580, 580) pivot col — North outbound col 3
    (S_LT, EXIT_N),    // (580, 290) exit north
];
